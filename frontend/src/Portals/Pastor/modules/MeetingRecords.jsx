...
const PastorMeetingRecords = () => {
  ...
  const fetchMeetingsData = async () => {
    try {
      ...
      console.error('Failed to load pastor meeting data:', err);
      setError('Failed to fetch meeting records. Please try again.');
    } finally {
      setLoading(false);
    }
  };
  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center py-20 text-slate-500">
        ...
        <p className="text-sm font-semibold">Loading Pastor Meeting Records...</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6 font-sans text-slate-800 antialiased">
        ...
        <div className="space-y-5 print:hidden">
          <div className="bg-white p-4 sm:p-5 rounded-2xl border border-slate-200/80 shadow-xs flex flex-col md:flex-row items-center justify-between gap-4">
            <div className="w-full md:w-auto">
              ...
              <p className="text-xs font-semibold text-slate-500 mt-0.5">Filter, preview, download, or print official meeting records and archived docs.</p>
            </div>
      ...
      {/* DRAWER: CLICKABLE MEETING DETAILS */}
      {selectedMeeting && (
...
export default PastorMeetingRecords;